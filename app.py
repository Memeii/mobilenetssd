...
lineaccesstoken = 'kyJKHQnmAUkBVoG3T60Eol+5/F2tED8FUedDlgmXQIrceksn0xsyyR3B44Hicj8gNUQ6dOEIwp0xorHlicLGKv9SGUCp5v21f59Bbw3U9cNYqOsSZKEBIhe4PQ9JPiDSWEYNAbjil2wZyjh3ygWN1gdB04t89/1O/w1cDnyilFU='
...
def event_handle(event,json_line):
    ...
    if msgType == "text":
        ...
        if (msg == "สวัสดี") :
            replyObj = TextSendMessage(text="สวัสดีคั้บบ")
            line_bot_api.reply_message(rtoken, replyObj)
        elif (msg == "ไปเที่ยวกันไหม") :
            replyObj = TextSendMessage(text="ไปกันนนน")
            line_bot_api.reply_message(rtoken, replyObj)
        elif (msg == "ไปกินข้าวกันไหม") :
            replyObj = TextSendMessage(text="ไปสิ")
            line_bot_api.reply_message(rtoken, replyObj)
        elif (msg == "รักน้า") :
            replyObj = TextSendMessage(text="กอดๆ")
            line_bot_api.reply_message(rtoken, replyObj)
        else :
            headers = request.headers
            json_headers = ({k:v for k, v in headers.items()})
            json_headers.update({'Host':'bots.dialogflow.com'})
            url = "https://dialogflow.cloud.google.com/v1/integrations/line/webhook/cbf4a958-aa7b-4d40-830c-74d631197cca"
